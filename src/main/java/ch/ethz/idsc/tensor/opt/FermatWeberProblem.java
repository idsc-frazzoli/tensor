...
public class FermatWeberProblem implements Serializable {
  ...
  public Tensor weiszfeld() {
    while (iteration < iteration_max) {
      ...
      ++iteration;
      double delta = Norm._2.vector(point.subtract(next)).number().doubleValue();
      point = next;
      if (delta <= tolerance)
        break;
    }
  ...
  private Tensor weiszfeldStep() {
    Tensor dist = Tensor.of(tensor.flatten(0).map(anchor -> Norm._2.vector(anchor.subtract(point))));
