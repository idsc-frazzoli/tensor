...
public class MultinomialTest extends TestCase {
  public void testEmptyReal() {
    Scalar actual = Multinomial.horner(Tensors.empty()).apply(RealScalar.of(2));
    assertEquals(actual, RealScalar.ZERO);
  }

  public void testEmptyGaussian() {
    Scalar actual = Multinomial.horner(Tensors.empty()).apply(GaussScalar.of(4, 7));
    assertEquals(actual, GaussScalar.of(0, 7));
  }

  public void testHorner1() {
    Tensor coeffs = Tensors.vector(-3, 4);
    Scalar actual = Multinomial.horner(coeffs).apply(RealScalar.of(2));
  ...
  public void testGauss() {
    Scalar scalar1 = Multinomial.horner(Tensors.of( //
  ...
  public void testAccumulate() {
    ...
    assertEquals(Multinomial.horner(coeffs).apply(RealScalar.ONE), Total.of(coeffs));
    assertEquals(Last.of(accumu), Total.of(coeffs));
    for (int index = 1; index < coeffs.length(); ++index) {
      Scalar scalar = Multinomial.horner(coeffs.extract(index, coeffs.length())).apply(RealScalar.ONE);
  ...
  public void testQuantity() {
    ...
    Scalar res = Multinomial.horner(Tensors.of(qs1, qs2)).apply(val);
