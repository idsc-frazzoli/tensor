...
public class FermatWeberProblemTest extends TestCase {
  ...
  public void testSimple2() {
    ...
    Tensor sol = fwp.weiszfeld();
    assertTrue(fwp.getIterationCount() < 15);
    assertTrue(Norm._2.vector(sol.subtract(tensor.get(1))).Get().number().doubleValue() < 2e-2);
  }
  ...
  public void testWeighted() {
    ...
    Tensor sol = fwp.weiszfeld();
    assertTrue(fwp.getIterationCount() < 15);
    assertTrue(Norm._2.vector(sol.subtract(tensor.get(0))).Get().number().doubleValue() < 2e-2);
  }
}
